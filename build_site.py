...
@media (max-width:767px){.grid2{grid-template-columns:1fr}.mapcard{position:static}#map{height:min(62vh,520px);min-height:340px}.brand span{display:none}.top{position:static}.top .in{padding:var(--s2) var(--s4)}nav button{padding:6px 10px}.maplegend{width:184px;font-size:12px;padding:6px 8px}.maplegend .lg-d{display:none}}
...
#panel .bar.neg{fill:var(--hi)}#panel .bar.pos{fill:var(--lo)}#panel .bar.soft{opacity:.42}
...
<label>색 기준 <select id="mode"><option value="mult">위험 배수 (모형 M3L)</option><option value="rate">실제 폐업률 (전국 대비)</option></select></label>
</div>
<div class="chips" id="bizbar"></div>
<div class="hint" id="askhelp" style="margin:0 0 var(--s3)">질문 형식은 <b>지역 + 업종</b>, <b>지역만</b>, <b>업종 + 위험한/안전한 곳</b> 세 가지입니다(LLM이 아니라 정해진 형식만 이해합니다). 예시: <button class="chip-b" data-q="동탄 서양음식">동탄 서양음식</button> <button class="chip-b" data-q="합천 한식">합천 한식</button> <button class="chip-b" data-q="강남구">강남구</button> <button class="chip-b" data-q="한식 위험한 곳">한식 위험한 곳</button> <button class="chip-b" data-q="경남 한식 안전한 곳">경남 한식 안전한 곳</button></div>
<div class="grid2">
...
function renderLegend(){
  ...
  $('#maplegend').innerHTML='<div class="lg-t">'+(r?'실제 폐업률 (전국 평균 대비)':'폐업 위험도 (평균 점포 대비)')+'</div><div class="lg-bar"></div>'+
   '<div class="lg-ticks"><i style="left:0"></i><i style="left:50%"></i><i style="right:0"></i><span style="left:0">×0.53↓</span><span style="left:50%;transform:translateX(-50%)">×1.0</span><span style="right:0">×1.9↑</span></div><div class="lg-r" style="justify-content:space-between;margin-top:0"><span>◀ 안전</span><span>위험 ▶</span></div>'+
...
function renderHome(){
  $('#panel').innerHTML='<p class="hint">지도의 버블을 누르거나 위 입력창에 <b>지역 + 업종</b>(예: 동탄 서양음식)을 적어 보세요. 형식은 입력창 아래에 있습니다.<br><br>“왜”는 Cox 모형(M3L)의 선형예측자를 요인별로 정확히 쪼갠 값입니다. 붉은 막대는 위험을 높이는 요인, 푸른 막대는 낮추는 요인이며, 요인들을 곱하면 위험 배수가 됩니다.</p>';
}
function bizRows(r){
...
function say(html){$('#panel').innerHTML=html;$('#panel').querySelectorAll('[data-r]').forEach(x=>x.addEventListener('click',()=>{if(x.dataset.b!==undefined)setBizQuiet(+x.dataset.b);select(+x.dataset.r,true);}));}
...
function ask(){
  ...
  setBizQuiet(b);
  if(cand.length>1){S.sel=null;update();goRegions(cand);say('<p>여러 시군구가 맞습니다. 선택해 주세요.</p><div class="nb">'+cand.slice(0,14).map(i=>'<button data-r="'+i+'">'+D.regions[i].sido+' '+D.regions[i].name+'</button>').join('')+'</div>');return;}
  select(cand[0],true);
}
