...
def step2(h):
    ...
    h = _rep(h, "<h3>가장 안전한 8개 지역·업종 조합</h3>", '<h3>가장 안전한 8개 지역·업종 조합</h3>')
...
PLAIN = [   # 제가 쓴 "~요"체 문장 → "~이다"체 (원문 "~입니다"체는 아래 규칙으로 처리)
    ...
    ("비교한 폐업 위험도예요. 오른쪽 요인 배수들(표에서 뺀 입지·직전 폐업률 요인 포함)을 곱한 값이에요. 연관일 뿐 원인은 아니에요.", "비교한 폐업 위험도이다. 오른쪽 요인 배수들(표에서 뺀 입지·직전 폐업률 요인 포함)을 곱한 값이다. 연관일 뿐 원인은 아니다."),
]
...
def plain_style(h):
    for old, new in PLAIN:
        assert old in h, f"말투 변환: 원문 없음 {old[:40]!r}"
        h = h.replace(old, new)
    return _hap(h)
