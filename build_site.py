...
(function(){ // 두 지역 비교: 같은 업종 기준으로 요인 막대를 같은 순서로 나란히 놓는다
  ...
  function show(){
    ...
    if(a<0||b<0){out.innerHTML='<p class="hint">'+((!$('#cmpA').value.trim()&&!$('#cmpB').value.trim())?'두 지역을 고르면 폐업 위험도와 요인 막대를 나란히 보여 줘요.':'지역 이름을 목록에서 골라 주세요. 이름이 여러 곳과 겹치면 “서울특별시 마포구”처럼 시도까지 써 주세요.')+'</p>';return;}
    ...
    out.innerHTML='<div class="cmp">'+card(a,va,order)+card(b,vb,order)+'</div><p class="cmpdiff">두 지역의 가장 큰 차이는 <b>'+dk.d.label+'</b>이에요 ('+rname(a)+' ×'+dk.d.m.toFixed(2)+' · '+rname(b)+' ×'+dk.e.m.toFixed(2)+'). 요인 배수를 모두 곱하면 각 지역의 폐업 위험도가 돼요.</p>'+
      '<p class="cap">같은 업종 기준('+(bz>=0?BIZ[bz]:'전체 업종')+')이며 통계적 연관이지 원인은 아니에요. 옅은 막대(BC카드 고객 연령대)는 참고용이라 차이 비교에서 뺐어요.</p>';
